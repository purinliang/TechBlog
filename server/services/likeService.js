...
const LikeService = {
  getPostIdsLikedByUser: async (userId) => {
    ...
    await cacheClient.setEx(key, getRandomTTL(), JSON.stringify(likedPostIds));
  ...
  getLikeCountForPost: async (postId) => {
    ...
    await cacheClient.setEx(key, getRandomTTL(), String(count));
  ...
  addLike: async (postId, userId) => {
    ...
    await cacheClient.del(LIKES_CACHE_KEY(userId));
    await cacheClient.del(LIKE_COUNT_CACHE_KEY(postId));
